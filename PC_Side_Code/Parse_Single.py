...
def convert_bin_to_binary_string(filename):
    """
    Reads a binary file and returns a string of its binary representation.
    Each byte is converted to an 8-bit binary string, separated by a space for readability.
    """
            ...
            while True:
                ...
                while count < Sample_Size:
                    ...
                    if not byte_data:
                        ...
                        return '\n'.join(Binary_Samples)
                    # Convert the byte (integer value from 0 to 255) to an 8-bit binary string
                    # format(byte_data[0], '08b') uses the format function to ensure 8 bits and padding with zeros
                    binary_representation = format(byte_data[0], '08b')
                    Sample.append(binary_representation)
                Sample_Str = ' '.join(Sample)
                Binary_Samples.append(Sample_Str)
    ...
    # Join the 8-bit binary strings with spaces
    print("Debug1")
    print(Binary_Samples[0])
    return '\n'.join(Binary_Samples)
...
def Convert_Bin(Path):
    binary_output = convert_bin_to_binary_string(Path)
